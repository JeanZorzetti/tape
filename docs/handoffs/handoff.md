...
**Execução da prospecção (feature 005):** põe o playbook *dentro* do CRM — o representante não precisa mais alternar entre o doc e a tela. **Sem schema novo, sem dependência nova, zero-JS** (reusa `tentativas`, `NICHOS`, `CADENCIA_DIAS`, `whatsappParaNumero`).
- **Script na ficha (US1)** — `/admin/[id]`, só pipeline **recuperação** e só enquanto a cadência de 1ª venda está em cena (`!temPedido`, sai junto com ela quando o lead vira cliente). Mostra o script do **nicho × passo** (`passoDoLead = min(nToques, 5)` → 6 passos alinhados a `CADENCIA_DIAS`), com empresa/nome já preenchidos, e a **ação de contato na ordem**: telefone → botão WhatsApp com a mensagem pronta; senão Instagram → link do perfil + DM copiável (o Instagram **não** aceita DM pré-preenchida por URL); senão só o texto copiável. Objeções em `<details>` (sem JS).
- **Ritmo do dia (US2)** — `/admin` mostra **"N / META_TOQUES_DIA toques hoje"**, escopado à pipeline selecionada. Meta batida ganha rótulo **"✓ meta batida"** (texto, não só cor — FR-012).
- `src/lib/scripts.ts` (**novo**) — `SCRIPTS` por nicho (só o 1º toque muda; os 5 follow-ups são compartilhados), `passoDoLead`, `scriptDoLead`, `montarScript`, `OBJECOES`, `LABEL_PASSO`.
- `src/lib/adminUi.ts` — `+META_TOQUES_DIA` (default **20**, override por env; sempre > 0) e `+instagramUrl()` (normaliza `@x` / `x` / `instagram.com/x` / link → URL; `null` se implausível).
- `src/lib/crm.ts` — `+contarToquesHoje(pipeline)`: conta `tentativas` de hoje **no fuso America/Sao_Paulo** (converte no `where`, não depende do fuso do servidor Postgres) com `join leads` para escopar por pipeline.

⚠️ **Fonte dos scripts:** o conteúdo de verdade é `src/lib/scripts.ts`. [`docs/estrategia-prospeccao.md`](../estrategia-prospeccao.md) §4 é a **origem editorial** (documento narrativo paralelo, não lido em runtime) — editar os dois quando o texto mudar; não há sincronização automática, e isso é deliberado (`ponytail:` no topo do módulo). O playbook irmão navegável é **[`/admin/estrategia`](../../src/pages/admin/estrategia.astro)**.

⚠️ **`montarScript` nunca pode deixar `[placeholder]` cru** (SC-004) — recuperação quase nunca tem nome de contato (a planilha vem de fotos), então sem `nome` o vocativo é removido inteiro ("Oi, [nome]!" → "Oi!", sem vírgula órfã) e há uma varredura final que apaga qualquer `[...]` remanescente. Teste cobre isso.

**Verificado (005):** `npm test` **84 testes verdes** contra Postgres 16 em Docker (59 sem `DATABASE_URL`) — `scripts.test.mjs` (passo, placeholders, cobertura de todo nicho, `instagramUrl`) + `execucao-prospeccao.test.mjs` (integração: conta só hoje no fuso BR, escopa por pipeline; assere por **delta** para não quebrar com dados pré-existentes). `npm run build` OK. Smoke autenticado real: 1º toque com WhatsApp · lead com 1 toque avança para o follow-up D+1 · lead só com Instagram mostra "Abrir Instagram" + DM copiável · contador "1 / 20 toques hoje" escopado à recuperação · `<details>` das objeções abre sem JS · `<main>` sem overflow a 375px.

⚠️ **Achado fora do escopo de 005:** o **header do `AdminLayout` estoura na horizontal a 390px** (`scrollWidth` 644 × `clientWidth` 375) — são os 5 links de nav + seletor de pipeline + Sair, que não cabem. É **pré-existente** (nada de 005 toca o header; o `<main>` das telas novas fica limpo), mas quebra o piso de responsividade do `CLAUDE.md` em *todo* o `/admin`. Corrigir numa passada própria do layout (nav que quebra linha ou rola), não dentro de uma feature de conteúdo.
